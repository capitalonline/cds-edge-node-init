...
func K8sV17Run (k8s17InitJsonUrl string, initInfo *utils.InitData) {
	...
	// set network gateway
	if err := pkg.GatewayConfig(initInfo.Gateway, initInfo.PrivateNetDevice); err != nil {
	...
	case utils.K8sV17:
		...
		if err:= pkg.TunnalSetup(initInfo); err != nil {
			log.Fatalf("TunnalSetup: failed, err is: %s", err)
		}
	default:
